...
def fetch_all():
    conn = sqlite3.connect(DB_FILE)
    cur = conn.cursor()
    cur.execute('SELECT id, nama_siswa, biologi, fisika, inggrism, prediksi_fakultas FROM nilai_siswa ORDER BY id DESC')
    rows = cur.fetchall()
    conn.close()
    return rows
...
class NilaiApp:
    def __init__(self, root):
        self.root = root
        root.title('input nilai Siswa - sqlite3')
        ...
        try:
            ...
            pass
        styele.configure('TLabel', font=('Segoe UI', 10))
        ...
        frm_left.grid(row=0, column=0, padx=12, pady=12, sticky='nsw')

        frm_right = ttk.LabelFrame(root, text='Data Tersimpan', padding=(8, 8))
        frm_right.grid(row=0, column=1, padx=12, pady=12, sticky='nsew')
